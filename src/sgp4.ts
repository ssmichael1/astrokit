 ...
 * but his coding is very opaque and does not take advantage of modern
 * coding standards.
 * 
 * I am making no attempt to add clarity or make more efficient
...
// Make javascript look like c++
// for some key mathematical functions
const pi = Math.PI
...
export const getgravconst = (whichconst: string, satrec: any): void => {
	...
	switch (whichconst) {
		...
		case 'wgs72':
			...
			satrec.radiusearthkm = 6378.135;     // km
			satrec.xke = 60.0 /
				Math.sqrt(satrec.radiusearthkm * satrec.radiusearthkm * satrec.radiusearthkm / satrec.mus);
			satrec.tumin = 1.0 / satrec.xke;
			satrec.j2 = 0.001082616;
			satrec.j3 = -0.00000253881;
			satrec.j4 = -0.00000165597;
...
export const sgp4 = (satrec: any, tsince: number): SatRV | undefined => {
	...
	*/
	...
	if (pl < 0.0) {
		...
		/* --------- position and velocity (in m and m/sec) ---------- */
		r[0] = (mrt * ux) * satrec.radiusearthkm * 1000
		r[1] = (mrt * uy) * satrec.radiusearthkm * 1000
		r[2] = (mrt * uz) * satrec.radiusearthkm * 1000
		v[0] = (mvt * ux + rvdot * vx) * vkmpersec * 1000
		v[1] = (mvt * uy + rvdot * vy) * vkmpersec * 1000
		v[2] = (mvt * uz + rvdot * vz) * vkmpersec * 1000
	}  // if pl > 0
...
export const sgp4init = (whichconst: string, opsmode: string, satn: string, epoch: number,
	...
	*/
	getgravconst(whichconst, satrec)
